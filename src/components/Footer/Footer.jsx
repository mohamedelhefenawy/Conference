  ...
  return (
    <div className="max-w-[80%] mx-auto mt-24 flex flex-col items-center gap-12">
      ...
      <div className="flex gap-8 flex-wrap justify-center">
        <AnchorLink href="#home" onClick={() => useline('#home')} className={`text-2xl px-4 py-2 border border-[#54c08a] rounded-lg ${line === '#home' ? 'bg-[#54c08a] text-white' : 'text-[#384250]'}`}>
          الصفحة الرئيسية
        </AnchorLink>
        <AnchorLink href="#about" onClick={() => useline('#about')} className={`text-2xl px-4 py-2 border border-[#54c08a] rounded-lg ${line === '#about' ? 'bg-[#54c08a] text-white' : 'text-[#384250]'}`}>
          تعرف علينا
        </AnchorLink>
        <AnchorLink href="#menu" onClick={() => useline('#menu')} className={`text-2xl px-4 py-2 border border-[#54c08a] rounded-lg ${line === '#menu' ? 'bg-[#54c08a] text-white' : 'text-[#384250]'}`}>
          القائمة
        </AnchorLink>
        <AnchorLink href="#contact" onClick={() => useline('#contact')} className={`text-2xl px-4 py-2 border border-[#54c08a] rounded-lg ${line === '#contact' ? 'bg-[#54c08a] text-white' : 'text-[#384250]'}`}>
          تواصل معنا
        </AnchorLink>
      </div>
